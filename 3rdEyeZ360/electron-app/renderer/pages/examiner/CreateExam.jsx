...
const defaultForm = {
  ...
  end_time: '', duration_minutes: 120, threshold: 10,
  instructions: '', allowed_websites: [], allowed_applications: []
}
...
export default function CreateExam({ onBack, onCreated }) {
  ...
  const addApp = () => {
    ...
    if (!form.allowed_applications.includes(appInput.trim())) {
      set('allowed_applications', [...form.allowed_applications, appInput.trim()])
    }
    setAppInput('')
  }
  ...
  const handleSave = async (status = 'Published') => {
    ...
    setLoading(true)
    try {
      const res = await axios.post(`${API}/api/exams`, {
        ...form,
        ...
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          ...
          {errors.submit && (
            <div style={{
              ...
              borderRadius: 8, padding: '10px 16px', color: '#f75f5f',
          ...
          <div style={{
            ...
            <Field label="Description">
              <textarea value={form.description} onChange={e => set('description', e.target.value)}
                rows={3} placeholder="Brief description of this exam..."
                style={{
                  background: '#22263a', border: '1px solid #2e3347',
                  borderRadius: 8, padding: '8px 12px', color: '#e8eaf0',
                  fontSize: 14, width: '100%', resize: 'vertical', outline: 'none'
                }} />
            </Field>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <Field label="Violation Threshold">
          ...
          <div style={{
            background: '#1a1d27', border: `1px solid ${errors.websites ? '#f75f5f' : '#2e3347'}`,
            borderRadius: 14, padding: 24, marginBottom: 20
          }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              <input value={websiteInput} onChange={e => setWebsiteInput(e.target.value)}
                ...
                style={{ flex: 1 }}
              />
              <button onClick={addWebsite} className="btn btn-primary"
                style={{ padding: '8px 16px', fontSize: 13 }}>Add</button>
            </div>
            {form.allowed_websites.length === 0 ? (
              <div style={{ fontSize: 12, color: '#8b90a0', padding: '10px 0' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {form.allowed_websites.map(url => (
                  <div key={url} style={{
                    ...
                    color: '#34c97a', display: 'flex', alignItems: 'center', gap: 8
                  }}>
                    🔗 {url}
                    <button onClick={() => removeWebsite(url)} style={{
                      background: 'none', color: '#34c97a', fontSize: 14,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              <input value={appInput} onChange={e => setAppInput(e.target.value)}
                ...
                style={{ flex: 1 }}
              />
              <button onClick={addApp} className="btn btn-ghost"
                style={{ padding: '8px 16px', fontSize: 13 }}>Add</button>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {form.allowed_applications.map(app => (
                <div key={app} style={{
                  background: '#22263a', border: '1px solid #2e3347',
                  borderRadius: 20, padding: '5px 12px', fontSize: 12,
                  color: '#c8cad0', display: 'flex', alignItems: 'center', gap: 8
                }}>
                  {app}
                  <button onClick={() => removeApp(app)} style={{
                    background: 'none', color: '#8b90a0', fontSize: 14,
                    cursor: 'pointer', lineHeight: 1, padding: 0
                  }}>×</button>
                </div>
              ))}
          ...
          <div style={{
            ...
            <textarea value={form.instructions}
              ...
              style={{
                ...
                fontSize: 14, width: '100%', resize: 'vertical', outline: 'none', lineHeight: 1.7
              }}
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
            <button onClick={() => handleSave('Draft')} disabled={loading}
              className="btn btn-ghost" style={{ padding: '11px 24px', fontSize: 14 }}>
              Save as Draft
            </button>
            <button onClick={() => handleSave('Published')} disabled={loading}
              className="btn btn-primary" style={{ padding: '11px 28px', fontSize: 14 }}>
